    ...
    const renderDescription = (description) => {
        ...
        return (
            <div className="w-full">
                <span className="text-gray-600 text-xs flex-1 whitespace-normal" style={{
                    wordBreak: 'normal',
            ...
            <div className="flex items-center hover:cursor-pointer">
                {[...Array(5)].map((_, i) => (
                    <svg
                        ...
                        viewBox="0 0 24 24"
                        className={`h-3 w-3 tablet:w-5 tablet:h-5 ${i < Math.round(averageRating) ? 'text-yellow-400' : 'text-gray-300'}`}
                        fill="currentColor"
                        stroke="currentColor"
                        strokeWidth="1.5"
                        strokeLinecap="round"
                ...
                <span className="ml-2 test-xs tablet:text-base text-gray-800">{averageRating.toFixed(1)}</span>
            </div>

            <div className='flex justify-between gap-2 items-center mt:2 tablet:mt-4'>
                <button
